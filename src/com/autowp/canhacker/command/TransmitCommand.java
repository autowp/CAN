...
public class TransmitCommand extends Command {
    ...
    public TransmitCommand(int id, byte[] newData) throws CommandException
    {
        this.name = "t";
        
        if (id > 0x07FF) {
            throw new CommandException("ID cannot be greater than 11 bits int");
        }
        
        this.id = id;
        
        if (newData.length > MAX_DATA_LENGTH) {
            throw new CommandException("Data length must be in " + MAX_DATA_LENGTH + " bytes");
        }
